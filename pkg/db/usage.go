...
func BuildUsageWhere(provider, period, startDate, endDate string) (string, []interface{}) {
	...
	if startDate != "" || endDate != "" {
		...
		case "": // Frontend default = Today (start of local calendar day, computed in UTC).
			clauses = append(clauses, "datetime(timestamp) >= datetime('now', 'start of day')")
		case "day", "1d", "24h", "today": // Rolling last 24h / explicit today label.
			clauses = append(clauses, "datetime(timestamp) >= datetime('now', '-1 day')")
		case "yesterday":
			clauses = append(clauses, "datetime(timestamp) >= datetime('now', '-1 day', 'start of day') AND datetime(timestamp) < datetime('now', 'start of day')")
		case "week", "7d":
			clauses = append(clauses, "datetime(timestamp) >= datetime('now', '-7 days')")
		case "month", "30d", "1m":
			clauses = append(clauses, "datetime(timestamp) >= datetime('now', '-30 days')")
		case "all":
			// No date filter — the entire history.
		}
	}
